...
const addToHistory = (appSdk, storeId, auth, productId, variationId, itemBody) => {
  ...
  for (let field in itemBody) {
    if (itemBody.hasOwnProperty(field)) {
      switch (field) {
        case 'currency_id':
...
module.exports = (client, product, removeAll = false, appConfig) => {
      ...
      if (appConfig.disable_fix_product_price !== true) {
        ...
        if (variations && variations.length) {
          ...
          for (let i = 0; i < variations.length; i++) {
            if (variations[i].hasOwnProperty('price')) {
        ...
        if (priceChanged) {
          ...
          setTimeout(() => {
            ...
            appSdk.apiRequest(storeId, url, method, data, auth, noAuth).then(({ response }) => {
              let productBody = response.data
              if (variation) {
                // check if variation exists on product body
                if (productBody.variations) {
                  let variationBody = product.variations.find(body => body._id === variation._id)
                  // check if handling the current variation price
                  if (variationBody && variationBody.price === variation.price) {
                    addToHistory(appSdk, storeId, auth, product._id, variation._id, variationBody)
                  }
                }
              } else if (productBody.price === product.price) {
                // main product price edited
                addToHistory(appSdk, storeId, auth, product._id, null, productBody)
              }
            }).catch(errorHandling)
          }, 60000)
        }
