      ...
      if (order.fulfillment_status || order.financial_status) {
        if (appConfig.disable_fix_order_status !== true) {
          ...
          ;(async function delayed () {
            ...
            // check if fullfilment and/or financial status was setted to handle records check
            if (order.fulfillment_status && !order.fulfillment_status.startsWith('partial')) {
              mustCheckRecords.push('fulfillments')
            }
            if (order.financial_status && !order.financial_status.startsWith('partial')) {
              mustCheckRecords.push('payments_history')
            }
            if (mustCheckRecords.length) {
