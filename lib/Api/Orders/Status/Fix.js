...
module.exports = (client, order, removeAll = false, appConfig, fullOrder) => {
  return new Promise(resolve => {
    const end = () => resolve(client, order, removeAll, appConfig, fullOrder)
    const fn = appConfig => {
          ...
          ;(async function delayed () {
            ...
            if (fullOrder) {
              ...
              await appSdk.apiRequest(storeId, url, method, data, auth, noAuth).then(({ response }) => {
                // overwrite full order body object
                orderBody = fullOrder = response.data
              }).catch(errorHandling)
            }
