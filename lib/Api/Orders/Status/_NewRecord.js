      ...
      appSdk.apiRequest(storeId, url, method, data, auth).then(({ response }) => {
        ...
        if (lastStatusRecord && lastStatusRecord._id === statusRecord._id) {
          ...
          if (!lastStatus || !orderBody[fieldStatus] || orderBody[fieldStatus].current === lastStatus) {
            ...
            // send authenticated API request to edit handled status record
            appSdk.apiRequest(
              storeId,
              `/orders/${order._id}/${fieldRecords}/${lastStatusRecord._id}.json`,
              'PATCH',
              { flags: flags.unshift('cpm:patch') },
              auth
            ).catch(errorHandling)
