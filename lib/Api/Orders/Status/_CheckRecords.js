      ...
      if (status) {
        ...
        if (records) {
          ...
          if (lastRecord) {
            ...
            if (!Array.isArray(lastRecord.flags) || lastRecord.flags.indexOf('cpm') === -1) {
              // check if last record is recent by date (one hour)
              if (!lastRecord.date_time || Date.now() - new Date(lastRecord.date_time).getTime() < timeDiff) {
                continue
              }
            }
          }
